...
import $ from 'jquery';
import {haveSameContent} from '../common/utility/arrays';

type EntityTypesMap = {
  [entityType: CoreEntityTypeT]: string | Array<string>,
};
...
export type RelationshipTypeT =
  | string // Single type
  | Array<string>; // A type combination
 ...
 *
 * e.g: multiple(LINK_TYPES.otherdatabases, LINK_TYPES.lyrics)
 * returns: {
 *  release: [LINK_TYPES.otherdatabases.release, LINK_TYPES.lyrics.release],
 *  ...
 * }
 */
...
export class Checker {
  ...
  guessType(): RelationshipTypeT | false {
    ...
    if (cleanup && cleanup.select) {
      ...
      // `select` function may return false if it can't determine
      if (result) {
        return result;
      }
    }
  ...
  checkRelationships(
    ...
    allowedTypes: Array<RelationshipTypeT> | false,
  ): ValidationResult {
    if (!allowedTypes) {
      return {result: true};
    }
    ...
    const result = allowedTypes.some(
      (allowedType) => typeof allowedType === 'object' &&
        haveSameContent(selectedTypes, allowedType),
    );
    if (!result) {
      return {
        result: false,
